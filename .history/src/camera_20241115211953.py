import eventlet
eventlet.monkey_patch()  # Apply monkey patch to make it compatible with WebSocket
...
# Enable CORS for the entire app (allow requests from React)
CORS(app)

# Initialize SocketIO with CORS settings
socketio = SocketIO(app, cors_allowed_origins="http://localhost:5173")  # Allow requests from React
...
class VideoCamera:
    ...
    def alarm(self, msg):
        print(f"Sending alert: {msg}")  # Debugging: Check if the alert is being triggered
        socketio.emit('alert', {'message': msg}, broadcast=True)
        self.saying = False
