  ...
  useEffect(() => {
    const readAlertFile = async () => {
      try {
        ...
        setAlert(lines[lines.length - 1].trim() || "No alerts yet!"); // Set the last line of the file as alert
      } catch (error) {
    ...
    const interval = setInterval(readAlertFile, 5000); // Poll the file every 5 seconds
